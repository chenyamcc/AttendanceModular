	...
	$(".details").click(function(){
		...
		$.ajax({
			...
			success: function(data) {
				...
				var applytime = new Date(data.applytime);
				var rid
				$("#overtimeDivModel #examine").val(data.amid);
				$("#overtimeDivModel #applytime").val(applytime.format('yyyy-MM-dd h:mm:ss'));
...
function examineStatus(choice) {
	...
	$.ajax({
		url: "/modifyApplyExamine",
		type: "POST",
		data:{amid:amid,asid:asid,rid:rid},
		success: function(data) {
			location.href='/applyExamine?rid='+data
		}, error: function(data) {
		}
	});
}
...
function overtimeHTML() {
	...
	overtimeHtml += '<label for="applyTime" class="col-sm-3 control-label">申领时间</label>';
	overtimeHtml += '<div class="col-sm-9">';
	overtimeHtml += '<input type="text" class="form-control" id="applytime"   readonly="readonly">';
	...
	overtimeHtml += '<div class="col-sm-9">';
	overtimeHtml += '<input type="text" class="form-control" id="des"  readonly="readonly">';
	overtimeHtml += '</div></div>';
	overtimeHtml += '<div class="form-group">';
